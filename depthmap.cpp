...
void depthmap::update_mat(Mat input1,Mat input2)
{
    img_ao_mat = input1;
    img_bo_mat = input2;
    if(incoltype == CV_LOAD_IMAGE_GRAYSCALE)
    {
        cvtColor(img_ao_mat,input1,CV_BGR2GRAY);
        cvtColor(img_bo_mat,input2,CV_BGR2GRAY);
    }
    sz = img_ao_mat.size();
    width_org = sz.width;
    height_org = sz.height;
    if(width_org * height_org < 30000)
    {
        int fratio = 5;
        int sel_oppoint = 3; // Default operating point
        // if (argc==5)         // Use provided operating point
        // sel_oppoint=atoi(argv[4]);
        // //cout<<sel_oppoint<<endl;
        switch (sel_oppoint)
        {
            case 1:
